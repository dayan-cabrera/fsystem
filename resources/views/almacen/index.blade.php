                    ...
                    @foreach ($almacenes as $almacen)
                        ...
                        <tr class="{{ $fechaMant->lt($hoy) ? 'text-danger' : '' }}">
                            ...
                            <td id="btns" class="align-middle" style="padding: 1rem;">
                                <div class="d-flex">
                                    <form action="{{ route('almacen.imprimir', $almacen->id) }}" method="get">
                                        <button class="btn btn-info btn-sm" type="submit" style="margin: 4px;"
                                            href="#">Descargar</button>
                                    </form>
                                    <form action="{{ route('estante.index', $almacen->id) }}" method="get">
                                        @csrf
                                        <button style="margin: 4px;" class="btn btn-sm btn-info">Esatantes</button>
                                    </form>

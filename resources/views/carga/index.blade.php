                ...
                <tbody>
                    @foreach ($cargas as $carga)
                        <tr>
                            ...
                            <td class="align-middle">Almacen: {{ $carga->almacen }} Estante: {{ $carga->estante }} Piso:
                                {{ $carga->piso }} Casilla:{{ $carga->id_casilla }}</td>
                            ...
                            <td id="btns" class="align-middle" style=" padding: 1rem;">
                                <div class="d-flex">
                                    <form action="{{ route('carga.imprimir', $carga->id) }}" method="get">
                                    ...
                                    <form action="{{ route('carga.destroy', $carga->id) }}" method="post">
                                        ...
                                        <button type="submit" style="margin: 4px;"
                                            class="btn btn-sm btn-danger">Sacar</button>
                                    </form>
                                </div>
